...
void UUTurnCombatComponent::BeginPlay()
{
	...
	if (OwningCharacter)
	{
		UE_LOG(LogTemp, Warning, TEXT("Composant attach� � : %s"), *OwningCharacter->GetName());
	}
	else
	{
		UE_LOG(LogTemp, Error, TEXT("Le propri�taire n'est pas un APaperCharacter !"));
	}
...
void UUTurnCombatComponent::StartTurn()
{
	bIsMyTurn = true;
	OwningCharacter = GetOwner();
	//UE_LOG(LogTemp, Warning, TEXT("D�but du tour pour l'acteur : %s"), *OwningCharacter->GetName());
	if (OwningCharacter)
	{
...
void UUTurnCombatComponent::EndTurn()
{
	...
	if (OwningCharacter)
	{
		...
		//ajoute 1 point d'action � l'acteur
		...
		if (StatsComp)
		{
			...
			{
				StatsComp->ActionPoints = StatsComp->MaxActionPoints; // Ne pas d�passer le maximum
			}
